<?php
/**
 * Group_item_model.php
 *
 * @copyright Copyright (c) 2019 AkiraXue
 * @author akira.xue <18862104333@163.com>
 * @created on 5/16/21 11:27 PM
 */

use Lib\Constants;
use Service\BaseModelTrait;

class Group_item_model extends MY_Model
{
    use BaseModelTrait;

    public $table = 'group_item';
    ...
    private function filterQuery(CI_DB_query_builder $query, array $params)
    {
        ...
        !empty($params['group_code']) && $query->where('group_code', $params['group_code']);

        !empty($params['unique_code']) && $query->where('unique_code', $params['unique_code']);

        !empty($params['no_unique_codes']) && $query->where_not_in('unique_code', $params['no_unique_codes']);
